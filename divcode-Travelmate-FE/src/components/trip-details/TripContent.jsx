...
import { CalendarDays, Map, PieChart, CheckSquare, Plus } from "lucide-react"
...
export default function TripContent({ trip }) {
  ...
  useEffect(() => {
    if (isModalOpen) {
      api
        .get("/api/trips/non-members?trip_id=" + trip.id)
        .then(data =>
          console.log("Fetched users:", setUsers(data.data.data || []))
        )
