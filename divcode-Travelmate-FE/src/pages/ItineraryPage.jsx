...
const ItineraryPage = () => {
  ...
  const [filteredItems, setFilteredItems] = useState(
    itineraryItems.filter(item => item.day_number === currentDay)[0]
      ?.activities || []
  )
  ...
  useEffect(() => {
    const fetchItineraryItems = async () => {
      ...
      setFilteredItems(
        data
          .filter(item => item.day_number === currentDay)[0]
    ...
    connectWebSocket(tripId, message => {
      if (
        message.event === "activity_created" ||
        message.event === "activity_deleted" ||
        message.event === "activity_updated"
      ) {
        fetchItineraryItems()
  ...
  const handleAddActivity = async () => {
    ...
    try {
      const add = await api.post(
        `/api/itinerary/${currentDay}/activities`,
        newItem
      )

      const response = await api.get(`/api/trips/${tripId}/itinerary`)
      setItineraryItems(response.data.data)
      setDays(response.data.data)
  ...
  return (
    <div className="container mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-travelmate-charcoal">
            Jadwal Perjalanan
          </h1>
              ...
              <div className="hidden md:flex md:flex-col space-y-2">
                {days.map(day => (
                  <Button
                    key={day.id}
                    variant={
                      currentDay === day.day_number ? "default" : "outline"
                    }
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <Input
              name="category"
              placeholder="Kategori (activity, transportation, etc.)"
