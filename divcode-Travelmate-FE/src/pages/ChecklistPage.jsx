...
const ChecklistPage = () => {
  ...
  useEffect(() => {
    ...
    connectWebSocket(tripId, message => {
      if (
        message.event === "todo_created" ||
        message.event === "todo_deleted" ||
        message.event === "todo_updated"
      ) {
        fetchChecklistItems()
  ...
  const completedCount = checklistItems.filter(item => item.status == "done")
    .length
    ...
    setChecklistItems(prev =>
      prev.map(item => {
        if (item.id === id) {
          ...
          api
            .put(`/api/trips/todos/${id}/edit`, { status: newStatus })
            .then(() => {
  ...
  const handleDeleteItem = id => {
    ...
    api
      .delete(`/api/trips/todos/${id}/delete`)
      .then(() => {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Track items to pack and tasks to complete
          </p>
        </div>
